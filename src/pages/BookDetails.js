...
function BookDetails() {
  ...
  useEffect(() => {
    axios.get(`https://localhost:3000/${bookId}`, { headers: { "x-api-key": token } }).then((responce) => { setValue(responce.data.data) })
      .catch((err) => alert(err.message))
  }, [])

  const deleteFun = () => {
    axios.delete(`https://localhost:3000/${bookId}`, { headers: { "x-api-key": token } }).then((responce) => {
